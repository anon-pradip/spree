...
When(
  ...
  async function (user, credentials) {
    ...
    await expect(page.locator(loginPage.loginCardTitlseSelector)).toHaveText(
      ["Login"],
    );
...
Then(
  "user {string} should be navigated to the admin panel dashboard",
  async function (user) {
...
When(
  "user {string} tries to log in with following credentials",
  async function (user, credentials) {
    credentials = credentials.hashes();
...
Then("error message {string} should be shown", async function (errorMessage) {
  const errMessage = await page
    .locator(loginPage.errorMessageSelector)
    .textContent();
